import Image from "next/image";
import Link from "next/link";
import { interactiveProjects } from "@/lib/data";
import InteractiveCTA from "@/components/InteractiveCTA";

export const metadata = {
  title: "Interactive — Games & Apps by Ben Childs",
  description: "Interactive games and apps developed by Ben Childs, Christchurch creative professional. Built with Unity, available on Steam, Google Play, and iOS.",
  alternates: { canonical: "https://www.benchilds.co.nz/interactive" },
};

export default function Interactive() {
  return (
    <>
      {/* Header */}
      <section className="py-20 px-6 bg-gray-950 text-white text-center">
        <h1 className="text-5xl md:text-6xl font-bold tracking-tight mb-6">Interactive</h1>
        <p className="text-gray-300 max-w-lg mx-auto text-lg">
          Let&apos;s discuss your interactive or games project.
        </p>
      </section>

      {/* Projects */}
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto flex flex-col gap-16">
          {interactiveProjects.map((project) => (
            <div key={project.title} className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              {(() => {
                const innerLink = project.links.find(l => l.href.startsWith("/"));
                const imageEl = (
                  <div className="relative w-full h-64 rounded-2xl overflow-hidden bg-gray-100">
                    <Image
                      src={project.image}
                      alt={project.title}
                      fill
                      className={`${project.imageClassName ?? "object-cover"} transition-transform duration-300 group-hover:scale-105`}
                      unoptimized={project.image.endsWith(".gif")}
                    />
                  </div>
                );
                return innerLink
                  ? <Link href={innerLink.href} className="group block">{imageEl}</Link>
                  : imageEl;
              })()}
              <div>
                <h2 className="text-2xl font-bold mb-4">{project.title}</h2>
                <p className="text-gray-600 leading-relaxed mb-4">{project.description}</p>
                {project.note && (
                  <p className="text-sm text-gray-500 italic mb-4">{project.note}</p>
                )}
                <div className="flex flex-wrap gap-3">
                  {project.links.map((link) => (
                    link.href.startsWith("/") ? (
                      <Link
                        key={link.label}
                        href={link.href}
                        className="inline-block px-6 py-2 bg-gray-950 text-white text-sm font-semibold rounded-full hover:bg-gray-800 transition-colors"
                      >
                        {link.label}
                      </Link>
                    ) : (
                      <a
                        key={link.label}
                        href={link.href}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-block px-6 py-2 bg-gray-950 text-white text-sm font-semibold rounded-full hover:bg-gray-800 transition-colors"
                      >
                        {link.label}
                      </a>
                    )
                  ))}
                </div>
              </div>
            </div>
          ))}
        </div>
      </section>

      <InteractiveCTA />

      <div className="py-6 px-6 text-center">
        <Link href="/ventures" className="text-sm text-gray-500 hover:text-gray-900 transition-colors">
          ← Back to Ventures
        </Link>
      </div>
    </>
  );
}
